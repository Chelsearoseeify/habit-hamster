...
import { upsertCompletion, removeCompletion } from '@/lib/api'
  ...
  const toggleCompletion = useCallback((routineId: string, date: string, maxCount: number = 1) => {
    setCompletions((prev) => {
      ...
      if (!existing) {
        ...
        updated = prev.map((c) =>
          c.routineId === routineId && c.date === date ? { ...c, count: newCount } : c
        )
      } else {
        newCount = 0
  ...
  const setCompletionCount = useCallback((routineId: string, date: string, count: number) => {
    setCompletions((prev) => {
      ...
      if (count <= 0) {
        ...
        upsertCompletion(routineId, date, count).catch(console.error)
      }
      return updated
    })
  }, [])
